 ...
 */
public class HolepunchBridgeDevSupportManager extends DevSupportManagerBase {
    private boolean mIsSamplingProfilerEnabled = false;

    public HolepunchBridgeDevSupportManager(
    ...
    public void handleReloadJS() {
        ...
        ReactContext reactContext = getCurrentContext();
        if (reactContext != null) {
            reactContext.runOnJSQueueThread(new Runnable() {
                @Override
                public void run() {
                    long jsi = reactContext.getJavaScriptContextHolder().get();
                    HolepunchBindingsPackage.nativeReloadSignal(jsi);
                }
            });
        }
